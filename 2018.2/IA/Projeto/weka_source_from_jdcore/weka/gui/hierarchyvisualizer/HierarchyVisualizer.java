  ...
  implements ComponentListener
{
  ...
  int positionLeafs(Node node, int nPosX) {
    ...
    for (int i = 0; i < m_children.length; i++) {
      nPosX = positionLeafs(m_children[i], nPosX);
    }
    
    return nPosX;
  }
  
  double positionRest(Node node) { if (node.isLeaf()) {
      return m_fPosX;
    }
    double fPosX = 0.0D;
    for (int i = 0; i < m_children.length; i++) {
      fPosX += positionRest(m_children[i]);
    }
    fPosX /= m_children.length;
    m_fPosX = fPosX;
    return fPosX;
  }
  ...
  double positionHeight(Node node, double fOffSet) {
    if (node.isLeaf()) {
      m_fPosY = (fOffSet + m_fLength);
      return m_fPosY;
    }
    double fPosY = fOffSet + m_fLength;
    double fYMax = 0.0D;
    for (int i = 0; i < m_children.length; i++) {
      fYMax = Math.max(fYMax, positionHeight(m_children[i], fPosY));
    }
    m_fPosY = fPosY;
    return fYMax;
  }
  ...
  class Node
  {
    ...
    boolean isLeaf() { return m_children == null; }
    ...
    double draw(Graphics g) {
      if (isLeaf()) {
        ...
        double fPosX2 = -1.7976931348623157E308D;
        for (int i = 0; i < m_children.length; i++) {
  ...
  void parseNewick(String sNewick)
    throws Exception
  {
    m_sNewick = sNewick;
    ...
    positionRest(m_tree);
    m_fHeight = positionHeight(m_tree, 0.0D);
  }
  ...
  Node parseNewick2(String sStr) throws Exception
  {
    ...
    if (sStr.startsWith("(")) {
      ...
      m_children[0] = parseNewick2(sStr.substring(1, i1));
      m_children[0].m_Parent = node;
      String sStr2 = sStr.substring(i1 + 1, i2 > 0 ? i2 : sStr.length());
      m_children[1] = parseNewick2(sStr2);
      m_children[1].m_Parent = node;
      if (sStr.lastIndexOf('[') > sStr.lastIndexOf(')')) {
        sStr = sStr.substring(sStr.lastIndexOf('['));
        i2 = sStr.indexOf(']');
        if (i2 < 0) {
          Messages.getInstance();throw new Exception(Messages.getString("HierarchicalClusterer_ParseNewick2_Exception_Text_First") + sStr);
        }
      ...
      if (sStr.lastIndexOf(':') > sStr.lastIndexOf(')')) {
        ...
        m_fLength = new Double(sStr).doubleValue();
      } else {
        m_fLength = 1.0D;
      }
    }
      ...
      if (sStr.contains("["))
      {
        ...
        int i2 = sStr.indexOf(']');
        if (i2 < 0) {
          Messages.getInstance();throw new Exception(Messages.getString("HierarchicalClusterer_ParseNewick2_Exception_Text_Second") + sStr);
        }
      ...
      if (sStr.length() > 0) {
        if (sStr.indexOf(':') >= 0) {
          ...
          m_sLabel = sStr.substring(0, iColon);
          if (sStr.indexOf(':', iColon + 1) >= 0) {
            int iColon2 = sStr.indexOf(':', iColon + 1);
            m_fLength = new Double(sStr.substring(iColon + 1, iColon2)).doubleValue();
            m_fTmpLength = new Double(sStr.substring(iColon2 + 1)).doubleValue();
          } else {
            m_fLength = new Double(sStr.substring(iColon + 1)).doubleValue();
          }
        } else {
          m_sLabel = sStr;
          m_fLength = 1.0D;
