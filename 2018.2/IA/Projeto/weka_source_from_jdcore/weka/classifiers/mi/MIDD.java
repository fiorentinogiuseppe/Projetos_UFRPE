      ...
      for (int i = 0; i < m_Classes.length; i++) {
        ...
        for (int j = 0; j < nI; j++) {
          ...
          for (int p = 0; p < m_Data[i].length; p++) {
            numrt[(2 * p)] += (1.0D - exp) * 2.0D * (x[(2 * p)] - m_Data[i][p][j]) * x[(p * 2 + 1)] * x[(p * 2 + 1)] / exp;
            
            numrt[(2 * p + 1)] += 2.0D * (1.0D - exp) * (x[(2 * p)] - m_Data[i][p][j]) * (x[(2 * p)] - m_Data[i][p][j]) * x[(p * 2 + 1)] / exp;
          }
        }
        


        denom = 1.0D - Math.exp(denom);
        if (denom <= m_Zero) denom = m_Zero;
        for (int q = 0; q < m_Data[i].length; q++) {
          if (m_Classes[i] == 1) {
            grad[(2 * q)] += numrt[(2 * q)] * (1.0D - denom) / denom;
            grad[(2 * q + 1)] += numrt[(2 * q + 1)] * (1.0D - denom) / denom;
          } else {
            grad[(2 * q)] -= numrt[(2 * q)];
